 ...
 * string is not human readable, but can be used to establish connections.
 *
 * ## Why another URL model?
 *
 * We offer a new URL model to address problems that other URL implementations don't.
 *
 * ### Different URLs should be different
 *
 * Although they have different content, some URL implementations consider the following two URLs
 ...
 * that makes some URL implementations unusable for many things. They shouldn't be used as a [Map]
 * key or in a [Set]. Doing so is both inefficient because equality may require a DNS lookup, and
 * incorrect because unequal URLs may be equal because of how they are hosted.
 *
 * ### Equal URLs should be equal
 *
 * These two URLs are semantically identical, but some implementations disagree:
 *
 *  * http://host:80/
 *
 *  * http://host
 *
 * Both the unnecessary port specification (`:80`) and the absent trailing slash (`/`) cause some
 * implementations to bucket the two URLs separately. This harms their usefulness in collections.
 * Any application that stores information-per-URL will need to either canonicalize manually, or
 * suffer unnecessary redundancy for such URLs.
 *
 ...
 *
 * Some URI implementations are strict around what URLs they accept. They reject URLs like
 ...
 * ### Plus a modern API
 *
 * Some URL/URI classes predate builders and instead use telescoping constructors. For example,
 ...
 * some URL implementations it's possible to create an awkward URL like `http:/` with scheme and
 ...
 */
expect fun String.toHttpUrl(): HttpUrl

/**
 * Returns a new `HttpUrl` representing `url` if it is a well-formed HTTP or HTTPS URL, or null
 * if it isn't.
 */
expect fun String.toHttpUrlOrNull(): HttpUrl?

expect class HttpUrlBuilder {
    constructor()
    ...
    fun username(username: String): HttpUrlBuilder

    fun encodedUsername(encodedUsername: String): HttpUrlBuilder

    fun password(password: String): HttpUrlBuilder

    fun encodedPassword(encodedPassword: String): HttpUrlBuilder
    ...
    fun addPathSegment(pathSegment: String): HttpUrlBuilder
     ...
     */
    fun addPathSegments(pathSegments: String): HttpUrlBuilder

    fun addEncodedPathSegment(encodedPathSegment: String): HttpUrlBuilder
     ...
     */
    fun addEncodedPathSegments(encodedPathSegments: String): HttpUrlBuilder
    ...
    fun removePathSegment(index: Int): HttpUrlBuilder

    fun encodedPath(encodedPath: String): HttpUrlBuilder

    fun query(query: String?): HttpUrlBuilder

    fun encodedQuery(encodedQuery: String?): HttpUrlBuilder
    ...
    fun removeAllQueryParameters(name: String): HttpUrlBuilder

    fun removeAllEncodedQueryParameters(encodedName: String): HttpUrlBuilder

    fun fragment(fragment: String?): HttpUrlBuilder

    fun encodedFragment(encodedFragment: String?): HttpUrlBuilder
 ...
 * | `http://127.0.0.1`            | null                 |
 */
expect suspend fun HttpUrl.getTopPrivateDomain(): String?